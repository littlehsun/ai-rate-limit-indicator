...
class UnifiedMacOSTests(unittest.TestCase):
    ...
    def test_package_and_installer_target_unified_macos_app(self):
        ...
        self.assertLess(
            mac_installer.index("app-placeholder"),
            mac_installer.index("app_was_running=false"),
        )
    ...
    def test_settings_surface_save_errors_and_auto_click_selects_provider(self):
        ...
        self.assertIn('values["DISPLAY_PROVIDER"]', configuration)
        self.assertIn("validLegacyProvider == nil ? .auto : .custom", configuration)
    ...
    def test_multiple_menu_bar_providers_use_one_composite_brand_image(self):
        ...
        self.assertIn("let windows = snapshot.indicatorDisplayWindows", views)
        self.assertIn("if let resetWindow = snapshot.indicatorResetWindow", views)
